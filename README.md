...
索引包含文章标题、正文和代码，排除导航、评论及空白页面；配置见 `pagefind.yml`。`pnpm run dev` 不生成或提供搜索索引，验证搜索效果需使用构建预览。修改笔记后重新构建，索引才会更新。查询分词、匹配与排序使用 Pagefind 默认行为。构建后可运行 `pnpm run test:search` 验证检索回归用例。
