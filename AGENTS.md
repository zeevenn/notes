...
Assume the reader knows the relevant programming basics. Explain only what is needed to understand the topic or a non-obvious behavior. Omit preventive clarifications, obvious code narration, and setup or version commentary that does not affect the conclusion. Do not turn every clarification from a conversation into a paragraph in the note. Borrow clear reasoning from good tutorials without copying their from-zero pacing. Delete sentences whose removal loses no necessary information; do not replace them with another explanation of why they were unnecessary.
