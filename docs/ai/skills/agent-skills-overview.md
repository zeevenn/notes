...
Agent Skills 是一种轻量、开放的能力封装格式。一个 skill 通常是包含指令、脚本和资源的文件夹，用于把特定任务的知识、流程和工具调用方式交给 Agent。

从工程角度看，Skills 是 Agent 可以按需加载的可复用功能单元，位置接近插件、任务模板或小型工作流。
...
这种方式能减少默认上下文占用，同时保留按需展开复杂指令的能力。
...
Agent Skills 适合封装稳定、可复用、跨任务重复出现的操作流程。它们不替代底层 tools，而是在 tools 之上提供更高层的业务语义。复杂 skill 也可以通过 MCP 访问外部系统。
...
好的 skill 应该边界清晰、触发条件明确，并把长说明、脚本和参考材料拆到合适的位置，避免默认上下文膨胀。
