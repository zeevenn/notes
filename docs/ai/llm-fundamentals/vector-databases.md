...
向量相似度和 BM25 分数的含义与取值范围不同，不能默认直接加权。常见实现使用 RRF（Reciprocal Rank Fusion，倒数排名融合），也可以先校准两类分数后再按权重合并。详见 [RAG：检索与重排](../rag/retrieval-and-reranking.md)。
...
在 RAG 系统中，向量数据库可以承担语义检索层的角色，但不是必需组件。小规模数据可以使用内存索引，精确查询可能使用全文搜索、关系数据库或业务 API。详见 [RAG 章节](../rag/README.md)。
