...
登录宝塔面板，可以参考 [配置宝塔面板](configure-bt.md)。点击左侧网站 -> 添加站点。

![step1-add-site](https://raw.githubusercontent.com/dribble-njr/typora-njr/master/img/step1-add-site.png)

新增之后，点击设置 -> 配置文件。这三项按自己的情况配置，在后面 [git hooks](./automated-deployment.md#配置-hooks) 中会将 dist 文件推送到 `root` 配置项设置的文件夹下。
...
在 [搭建 git 私服](./build-git-server.md) 中已经搭建好了 git 私服，因此只需要初始化 `vuepress.git` 仓库即可。
