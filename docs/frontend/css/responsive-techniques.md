...
<div class="row">
  ...
  <div class="col-6 col-s-9">
    <h1>The City</h1>
    <p>
      Chania is the capital of the Chania region on the island of Crete. The city can be divided in two parts, the old
...
有关 `box-sizing` 属性的更多信息，请参阅 [盒模型](./box-model.md)。
...
行内的列都会向左浮动，因此会脱离页面的流程，其他元素的放置就好像这些列不存在一样。为了避免这种情况，添加一种样式来 [清除页面浮动](./float.md#清除浮动)：
  ...
  <body>
    ...
    <div class="row">
      ...
      <div class="col-9">
        <h1>The City</h1>
        <p>
          Chania is the capital of the Chania region on the island of Crete. The city can be divided in two parts, the
...
<html>
  ...
  <body>
    <p>
      Resize the browser window. When the width of this document is 800 pixels or less, the color is "lightblue",
...
1. 如果 `background-size` 属性设置为 `100% 100%`，背景图片就会拉伸以覆盖整个内容区域：
...
1. 如果 `background-size` 属性设置为 `cover`，背景图片将按比例覆盖整个内容区域。请注意，`cover` 将保持纵横比，背景图片的某些部分可能会被剪切：
...
根据[移动优先原则](#始终坚持移动端优先的设计理念)，基础样式面向手机，通过媒体查询叠加大屏样式：
...
更现代的方案：用 `clamp()` 无需媒体查询实现流式缩放：

```css
/* 字号在 16px 到 20px 之间随视口平滑变化 */
