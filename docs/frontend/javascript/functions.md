...
事实上，在使用 `function` 关键字定义（非箭头）函数时，可以在函数内部访问 `arguments` 对象（参见 [类数组对象](./array-like-objects.md)），从中取得传进来的每个参数值。

:::

::: warning

ECMAScript 中的所有参数都按值传递的。不可能按引用传递参数。如果把对象作为参数传递，那么传递的值就是这个对象的引用，详见 [原始值和引用值](./primitive-vs-reference.md)。
