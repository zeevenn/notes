---
title: JavaScript
article: false
icon: creative
star: true
---

这里记录 ECMAScript 语言语法、语义和内置对象。浏览器 API 与 Node.js API 分别放在对应的宿主环境目录。

## 目录

- [var、let、const 区别](./var-let-const.md)
- [数据类型](./data-types.md)
- [判断数据类型](./type-checking.md)
- [原始值和引用值](./primitive-vs-reference.md)
- [执行上下文](./execution-context.md)
- [对象转换方法](./type-conversion-methods.md)
- [基本引用类型](./basic-reference-types.md)
- [集合引用类型](./collection-types.md)
- [类数组对象](./array-like-objects.md)
- [对象属性](./object-properties.md)
- [原型与原型链](./prototype-chain.md)
- [创建对象](./creating-objects.md)
- [继承](./inheritance.md)
- [类](./class.md)
- [手写 new 操作符](./implement-new.md)
- [函数](./functions.md)
- [this 指向](./this-binding.md)
- [手写 call、apply、bind](./implement-call-apply-bind.md)
- [判断对象值相等](./object-equality.md)
- [浅拷贝与深拷贝](./shallow-deep-copy.md)
- [事件循环](./event-loop.md)

## 相关平台

- [浏览器](../../frontend/browser/README.md)
- [AJAX](../../frontend/browser/ajax.md)
- [DOM 事件监听](../../frontend/browser/event-listeners.md)
- [Node.js](../../backend/node/README.md)
