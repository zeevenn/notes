...
我在项目里遇到的“登录鉴权”，通常会贯穿登录和业务请求：用户先证明身份，系统建立登录状态，接口再根据当前用户和资源判断是否允许访问。我习惯沿着一次请求把它拆成认证、登录态、授权和会话管理四部分，这样更容易判断每一层该选什么方案。
...
MFA 通常是在主要登录方式之后再增加一次验证，不单独承担完整的账号登录流程。第三方登录场景中，我主要用 OIDC 获取用户身份；OAuth2 更关注客户端获得访问资源的授权。

## 登录态怎么选

| 方案 | 服务端如何确认身份 | 撤销方式 | 我会在什么情况下考虑 |
| --- | --- | --- | --- |
...
做同源 Web 应用时，我通常先考虑 Session + HttpOnly Cookie，服务端管理会话，浏览器按 Cookie 规则自动携带。资源服务较多且确实需要本地验签时，再考虑短时效 JWT Access Token；如果封号、权限变化或设备下线需要立即生效，我会保留服务端会话记录，或者使用 Opaque Token 集中校验。
